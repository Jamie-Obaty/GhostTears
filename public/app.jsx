    ...
    <svg viewBox="0 0 120 120" className={avatarClass} aria-label="avatar">
      <defs>
        <filter id="softGlow" x="-50%" y="-50%" width="200%" height="200%">
          <feDropShadow dx="0" dy="0" stdDeviation="2" floodColor="#63ffd4" floodOpacity="0.4" />
        </filter>
      </defs>
      <circle cx="60" cy="58" r="33" fill={faceFill} filter="url(#softGlow)" />
      {isNervous && !isEliminated && (
        <g>
          <path d="M90 26 C96 35, 96 41, 90 47 C84 40, 84 34, 90 26" fill="#7cc6ff" opacity="0.85" />
        </g>
      )}
      {!isEliminated && (
        <>
          <ellipse className="eye" cx="48" cy="52" rx="5" ry="6" fill={eyeColor} />
          <ellipse className="eye" cx="72" cy="52" rx="5" ry="6" fill={eyeColor} />
        </>
      )}
      {isEliminated && (
      ...
      {!isEliminated && isWinner && <path d="M44 71 Q60 86 76 71" stroke="#063020" strokeWidth="4" fill="none" strokeLinecap="round" />}
      {!isEliminated && !isWinner && <path d="M44 75 Q60 69 76 75" stroke="#063020" strokeWidth="4" fill="none" strokeLinecap="round" />}
      <path d="M36 98 C35 85, 85 85, 84 98" stroke={faceFill} strokeWidth="8" fill="none" strokeLinecap="round" />
    </svg>
  );
}
...
function PlayerCard({ player, isActive, isMe, isWinner, isShocked, timer }) {
  ...
  return (
    <div className={cn('ghost-panel rounded-3xl p-4 border transition-all', isActive && 'card-active')}>
      <div className="flex items-center justify-between">
        <div>
          <div className="text-lg font-semibold tracking-wide">{player.name}{isMe ? ' (You)' : ''}</div>
          <div className={cn('text-xs tracking-[0.22em] mt-1 uppercase', player.eliminated ? 'text-red-300' : 'text-slate-300')}>
            {player.eliminated ? 'Eliminated' : isWinner ? 'Winner' : isActive ? 'Active Turn' : 'Waiting'}
          </div>
        </div>
        <Avatar
          isActive={isActive}
      ...
      <PenaltyBar penalties={player.penalties} />
    </div>
  );
}
...
function App() {
  ...
  const displayString = roomState?.currentString || 'TYPE HERE';
        ...
        <div className="ghost-panel rounded-3xl border p-4 md:p-6">
          {!joined && (
            <form onSubmit={joinGame} className="max-w-xl mx-auto py-14">
              <h1 className="text-4xl md:text-5xl font-black text-center tracking-[0.25em] mb-4">GHOSTTEARS</h1>
              <p className="text-center text-slate-300 mb-7">Multiplayer word duel. Join from anywhere with a room code.</p>
          ...
          {joined && roomState && (
            <>
              ...
              <div className="ghost-panel border rounded-3xl h-44 md:h-52 flex items-center justify-center px-3 mb-4">
                <div className={cn('font-black tracking-[0.24em] text-center break-all', roomState.currentString ? 'text-4xl md:text-6xl text-slate-100' : 'text-4xl md:text-6xl text-slate-500')}>
                  {displayString}
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 mb-5 text-xs md:text-sm text-slate-300">
                <span className="ghost-panel rounded-full px-3 py-2">You: {me?.name || 'Spectator'}</span>
                <span className="ghost-panel rounded-full px-3 py-2">Status: {roomState.status}</span>
                <span className="ghost-panel rounded-full px-3 py-2">Turn: {isMyTurn ? 'Your turn' : 'Waiting'}</span>
                <span className="ghost-panel rounded-full px-3 py-2">Players: {roomState.players.length}</span>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {roomState.players.map((player) => {
                  ...
                  const isShocked = roomState.lastPenalty
                    && roomState.lastPenalty.playerId === player.id
                  ...
                  return (
                    <PlayerCard
                      key={player.id}
                      player={player}
                      isActive={roomState.activePlayerId === player.id && roomState.status === 'playing'}
